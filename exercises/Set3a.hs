...
firstHalf txt = take (halveIndex txt) txt
    where halveIndex str = if even (length str) then div (length str) 2 else div (length str) 2 + 1
...
capitalize txt = unwords (map capitalizeFirst (words txt))
...
(+|+) :: [a] -> [a] -> [a]
(+|+) first second = starter first ++ starter second
    where   starter (x:xs) = [x]
            starter [] = []
...
sumRights [] = 0
sumRights (x:xs) = case x of    Left a -> 0 + sumRights xs
                                Right b -> b + sumRights xs
