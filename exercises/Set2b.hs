...
oddFactorial 1 = 1
-- oddFactorial x = if even x then 1 * oddFactorial(x-1) else x * oddFactorial(x-1)
oddFactorial x
    | even x = 1 * oddFactorial(x-1)
...
leftpad text x = if x <= length text then text else leftpad (" " ++ text) x
...
smallestDivisor' x divider
    | mod x divider == 0    = divider
    | otherwise             = smallestDivisor' x (divider+1) 
...
isPrime x = if (x > 1 && smallestDivisor x == x) then True else False 
...
biggestPrimeAtMost x = biggestPrimeAtMost' x

biggestPrimeAtMost' y
    | isPrime y             = y
    | otherwise             = biggestPrimeAtMost' (y-1)
